...
public class ProductDAO  extends AbstractDAO<Product>{
	public List<Product> findProductsByFeatured(Boolean featured) {
		if (featured == null)
			throw new IllegalArgumentException("The featured argument is required");
		TypedQuery<Product> q = entityManager.createQuery("SELECT o FROM Product AS o WHERE o.featured = :featured",
				Product.class);
	...
	public List<Product> findProductEntries(Product product) {
		Query query = null;
		if (product.getCategory() != null && !product.getCategory().getId().isEmpty() ) {
			query = entityManager.createQuery(
					"SELECT o FROM Product o WHERE o.name like :name and o.category = :category", Product.class);
	...
	public List<Product> findProductByCategory(Category category) {
		Query query = null;
		query = entityManager.createQuery("SELECT o FROM Product o WHERE o.category = :category",
				Product.class);		
	...
	public void remove(Product product) {
		if (this.entityManager.contains(product)) {
			...
			Product attached = findEntity(product.getId());
